...
// clang-format off
__xdata __at (EP0_ADDR) uint8_t Ep0Buffer[16];
...
__data uint8_t ReportType;  // 保存报告类型
__data uint8_t ReportID;    // 保存报告ID
...
void USB_EP0_SETUP() {
    ...
    if (len == (sizeof(USB_SETUP_REQ))) {
        SetupLen =
            ((uint16_t)UsbSetupBuf->wLengthH << 8) | (UsbSetupBuf->wLengthL);
            ...
            USB_REQ_TYP_STANDARD) // Not standard request
        {
            ...
            // here is the commnunication starts, refer to usbFunctionSetup of
            // USBtiny or usb_setup in usbtiny
            ...
            case USB_REQ_TYP_CLASS: {
                ...
                case GET_LINE_CODING: // 0x21  currently configured
                    ...
                    break;
                case SET_CONTROL_LINE_STATE: // 0x22  generates RS-232/V.24
                                             // style control signals
                ...
                case HID_SET_REPORT:
                    // 保存报告类型和ID
                    ReportType = UsbSetupBuf->wValueH;
                    ReportID = UsbSetupBuf->wValueL;
                    // 检查报告类型（wValue高字节）
                    if (ReportType == 3) { // 3=特征报告
                        // 接收特征报告数据
                        if (SetupLen > 0) {
                            // 设置接收数据的缓冲区
                            pDescr = (__code uint8_t *)Ep0Buffer;
                            // 设置要接收的长度
                            len = SetupLen;
                        }
                    } else {
            ...
            case USB_GET_DESCRIPTOR:
                ...
                case 1: // Device Descriptor
                    pDescr = (__code uint8_t *)
                        DeviceDescriptor; // Put Device Descriptor into outgoing
                                          // buffer
                ...
                case 0x22:
                    if (UsbSetupBuf->wValueL == 0) {
                        ...
                        len = ConfigurationDescriptor.HID_KeyboardHID
                                  .HIDReportLength;
                    } else {
                        len = 0xff;
                    }
            ...
            case USB_CLEAR_FEATURE: // Clear Feature
                    ...
                    USB_REQ_RECIP_DEVICE) // Clear the device featuee.
                {
                    if ((((uint16_t)UsbSetupBuf->wValueH << 8) |
                        ...
                        if (ConfigurationDescriptor.Config.ConfigAttributes &
                            0x20) {
                            // wake up
                        } else {
                            len = 0xFF; // Failed
                           ...
                           USB_REQ_RECIP_ENDP) // endpoint
                {
                    ...
                    case 0x84:
                        UEP4_CTRL = UEP4_CTRL & ~(bUEP_T_TOG | MASK_UEP_T_RES) |
                                    UEP_T_RES_NAK;
                        break;
                    case 0x04:
                        UEP4_CTRL = UEP4_CTRL & ~(bUEP_R_TOG | MASK_UEP_R_RES) |
                                    UEP_R_RES_ACK;
                        break;
                    case 0x83:
                        UEP3_CTRL = UEP3_CTRL & ~(bUEP_T_TOG | MASK_UEP_T_RES) |
                                    UEP_T_RES_NAK;
                        break;
                    case 0x03:
                        UEP3_CTRL = UEP3_CTRL & ~(bUEP_R_TOG | MASK_UEP_R_RES) |
                                    UEP_R_RES_ACK;
                        break;
                    case 0x82:
                        UEP2_CTRL = UEP2_CTRL & ~(bUEP_T_TOG | MASK_UEP_T_RES) |
                                    UEP_T_RES_NAK;
                        break;
                    case 0x02:
                        UEP2_CTRL = UEP2_CTRL & ~(bUEP_R_TOG | MASK_UEP_R_RES) |
                                    UEP_R_RES_ACK;
                        break;
                    case 0x81:
...
void USBInterrupt() { // inline not really working in multiple files in SDCC
    if (UIF_TRANSFER) {
        ...
        case UIS_TOKEN_OUT: { // SDCC will take IRAM if array of function
                              // pointer is used.
            switch (callIndex) {
            case 0:
                EP0_OUT_Callback();
        ...
        case UIS_TOKEN_SOF: { // SDCC will take IRAM if array of function
                              // pointer is used.
            switch (callIndex) {
            case 0:
                EP0_SOF_Callback();
        ...
        case UIS_TOKEN_IN: { // SDCC will take IRAM if array of function pointer
                             // is used.
            switch (callIndex) {
            case 0:
                EP0_IN_Callback();
        ...
        case UIS_TOKEN_SETUP: { // SDCC will take IRAM if array of function
                                // pointer is used.
            switch (callIndex) {
            case 0:
                EP0_SETUP_Callback();
    ...
    if (UIF_BUS_RST) {
        ...
        UEP1_CTRL = bUEP_AUTO_TOG |
                    UEP_T_RES_NAK; // Endpoint 1 automatically flips the sync
        ...
        // UEP4_CTRL = UEP_T_RES_NAK | UEP_R_RES_ACK;  //bUEP_AUTO_TOG only work
        // for endpoint 1,2,3
    ...
    if (UIF_SUSPEND) {
        ...
        if (USB_MIS_ST & bUMS_SUSPEND) { // Suspend
            ...
            // SAFE_MOD = 0x55;
            // SAFE_MOD = 0xAA;
            // WAKE_CTRL = bWAK_BY_USB | bWAK_RXD0_LO;    // Wake up by USB or
            // RxD0 PCON |= PD; // Chip sleep SAFE_MOD = 0x55; SAFE_MOD = 0xAA;
            // WAKE_CTRL = 0x00;
